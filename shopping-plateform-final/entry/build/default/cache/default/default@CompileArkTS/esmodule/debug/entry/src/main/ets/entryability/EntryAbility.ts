...
import { CommonEventUtil } from "@bundle:com.atomicservice.6917609041042536136/entry/ets/common/CommonEventUtil";
...
export default class EntryAbility extends UIAbility {
    ...
    onCreate(): void {
        ...
        // Subscribe to youth mode–related common events.
    ...
    onWindowStageCreate(windowStage: window.WindowStage): void {
        ...
        try {
            ...
            hilog.error(this.domainId, this.logTag, `Failed to obtain the main window. errCode: ${err.code}, message: ${err.message}`);
            return;
        }
        ...
        windowClass.setWindowLayoutFullScreen(isLayoutFullScreen).then(() => {
            ...
            hilog.error(this.domainId, this.logTag, `Failed to setWindowLayoutFullScreen. code: ${err.code}, message: ${err.message}`);
        });
        ...
        // 4. Initialize StorageService for data persistence.
        try {
            StorageService.getInstance().init(this.context);
            hilog.info(this.domainId, this.logTag, 'StorageService initialized successfully.');
        }
        catch (error) {
            hilog.error(this.domainId, this.logTag, `Failed to init StorageService. code: ${error.code}, message: ${error.message}`);
        }
        windowStage.loadContent('pages/Index', new LocalStorage(), (err) => {
            if (err.code) {
                hilog.error(this.domainId, this.logTag, `Failed to load the content. errCode: ${err.code}, message: ${err.message}`);
                return;
            }
