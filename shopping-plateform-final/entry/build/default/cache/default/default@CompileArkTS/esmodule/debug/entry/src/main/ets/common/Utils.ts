...
export function cancelAuthorization() {
    ...
    const cancelRequest: authentication.CancelAuthorizationRequest = new authentication.HuaweiIDProvider().createCancelAuthorizationRequest();
...
export function showErrorMessage(context: UIContext, error: BusinessError) {
    const userCanceled: number[] = [
        ErrorCode.SHIPPING_ADDRESS_USER_CANCELED,
        ErrorCode.INVOICE_ASSISTANT_USER_CANCELED,
    ...
    hilog.error(domainId, logTag, `Failed to obtain data. BusinessError errCode: ${error.code}, Message: ${error.message}`);
    // Network exception.
    if (networkError.includes(error.code)) {
        ...
        context.showAlertDialog({
            message: { "id": 16777231, "type": 10003, params: [], "bundleName": "com.atomicservice.account_atomicservice_sample", "moduleName": "entry" },
            offset: { dx: 0, dy: -12 },
            alignment: DialogAlignment.Center,
            autoCancel: false,
            confirm: {
                value: { "id": 16777232, "type": 10003, params: [], "bundleName": "com.atomicservice.account_atomicservice_sample", "moduleName": "entry" },
                action: () => {
                }
            }
        });
        ...
        try {
            ...
            hilog.error(domainId, logTag, `Failed to show toast. BusinessError errCode: ${error?.code}, Message: ${error?.message}`);
        }
    }
}
// Copy the file.
